...
global $pdo;
$totalAdmins = (int) $pdo->query("SELECT COUNT(*) FROM Usuario WHERE TipoUsuario = 'admin'")->fetchColumn();
...
if ($totalAdmins === 0 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome === '' || $email === '' || strlen($senha) < 4) {
        ...
        if ($stmt->fetchColumn()) {
            $erro = 'Já existe uma conta com esse e-mail. Use outro e-mail pro admin.';
        } else {
            ...
            header('Location: ' . URL_BASE . '/usuario/login.php?instalado=1');
            exit;
        }
    }
}
    ...
    <div class="col-md-6 mx-auto">
        <div class="card p-4 p-md-5">
            <?php if ($totalAdmins > 0): ?>
                <p class="mb-0">Já existe um administrador configurado. Se você perdeu o acesso, peça pra alguém com acesso ao banco de dados redefinir a senha diretamente na tabela <code>Usuario</code> (procure a linha com <code>TipoUsuario = 'admin'</code>).</p>
                <a href="<?= URL_BASE ?>/usuario/login.php" class="btn btn-marca rounded-pill mt-3">Ir para o login</a>
            <?php else: ?>
                ...
                <form method="post">
                    ...
                    <div class="form-floating mb-3 tem-toggle-senha">
                        ...
                        <button type="button" class="btn-toggle-senha" data-alvo="instalarSenha" aria-label="Mostrar senha">
                            <i class="bi bi-eye"></i>
                        </button>
                    </div>
