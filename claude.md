...
> Atualizado em 2026-08-08: a regra acima é sobre **schema** (tabela/coluna) — isso nunca vira arquivo, é sempre automático. Script de **dado** pontual (seed de catálogo, reset, ajuste feito uma vez em produção) é outra coisa: fica em `migration/`, nomeado `AAAA-MM-DD_NN_descricao.sql`, versionado no git como histórico do que já rodei em produção. Não roda sozinho (sem acesso a terminal no host, ver 1.7) — eu rodo manualmente no phpMyAdmin, o arquivo fica só de registro.
